...
let userValue= +prompt("я загадал случайное значение от 0 до 100");
let counter=0;

do {
    if (randomValue > userValue){
        userValue = +prompt(`я загадал значение больше, чем ${userValue}` );
    }
    else if (randomValue < userValue){
        userValue = +prompt(`я загадал значение меньше, чем ${userValue}` );
    }
    counter++;

  } while (userValue!=randomValue);
  alert(`Правильно! Загаданное значение ${userValue}. Вы угадали с ${counter} попытки.` ); 
